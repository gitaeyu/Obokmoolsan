...
class Smart_App(QMainWindow, form_class):
    def __init__(self):
        ...
        self.cancel_btn.clicked.connect(self.order_cancel)
...
class Smart_App(QMainWindow, form_class):
    ...
    def select_menu(self):
        ...
        self.input_code = c.fetchall()
        c.execute(
            f"INSERT INTO ordermanage (메뉴명, 주문ID, 수량, 상태, 취소여부, 메뉴코드, 날짜) VALUES ('{self.menu_comboBox.currentText()}','{self.id_lineEdit.text()}',"
            f"'{self.maxmenu_comboBox.currentText()}', '주문', 'N', '{self.input_code[0][0]}','{order_date[0][0]}')");
        c.execute("SELECT *FROM ordermanage");
        self.compare_order2 = c.fetchall()
        print(self.compare_order2, '비교2')
    ...
    def add_bom(self):  # bom 추가하기
        ...
        c = conn.cursor()
        c.execute(
            f"SELECT count(*) FROM bom WHERE 메뉴코드='{self.add_code_comboBox.currentText()}' AND 요리='{self.menuname_lineEdit.text()}' AND 재료이름='{self.ingredient_lineEdit.text()}' ")
        bom_data = c.fetchall()
        print(bom_data[0][0])
        if bom_data[0][0] == 0:
            if add_code != "" and menuname != "" and add_ingre != "" and ingre_code != "" and unit_name != "" and unit_combo != "" and price_name != "":
                print('hoi')
                c.execute(
                    f"INSERT INTO bom (메뉴코드, 요리, 재료이름, 재료코드, 수량, 단위, 단가) VALUES ('{add_code}', '{menuname}', '{add_ingre}', '{ingre_code}', '{unit_name}', '{unit_combo}', '{price_name}')");
                print('sb')
            else:
                QMessageBox.about(self, "알림", "빈칸 입력하세요")
                # pass
    ...
    def log_in(self):  # 로그인창에서 로그인버튼 눌렀을때
        ...
        c = conn.cursor()
        c.execute(
            f"SELECT id FROM user_info WHERE BINARY id='{self.id_lineEdit.text()}' and password='{self.password_lineEdit.text()}'");
        id_count = c.fetchall()
        c.execute(f"SELECT name FROM user_info WHERE BINARY id='{self.id_lineEdit.text()}'")
        # print(id_count[0][0])
        admin = c.fetchall()
    ...
    def signin(self):  # 가입하기 눌렀을때
        ...
        if self.sign_id != "" and self.sign_password != "" and self.re_password != "" and self.sign_name != "" and self.sign_address != "":
            ...
            if compare_id[0][0] == 0:
                if self.sign_password == self.re_password:  # 비밀번호, 비밀번호 확인 다를때
                    c.execute(
                        f"INSERT INTO user_info (id, password, name, address) VALUES ('{self.sign_id}','{self.sign_password}','{self.sign_name}', '{self.sign_address}')")
                    QMessageBox.about(self, "알림", "회원가입완료")
                    self.login_stackedWidget.setCurrentIndex(0)
                else:
                    QMessageBox.about(self, "알림", "비밀번호가 다릅니다")
